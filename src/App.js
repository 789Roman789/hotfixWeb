...
const App = () => {
	...
	const [addition, setAddition] = useState(JSON.parse((localStorage.getItem('additionD') || 'null')) || {});
			...
			<Switch>
				...
				<Route path="/basket/:areaId/:itemId" exact>
					<Basket
						foodAreas={FOOD_AREAS}
						order={order}
						onAdditionChanged={({ itemId, time, self, faster }) => {
							const updatedAddition = { ...addition };

							if (itemId in updatedAddition) {
								updatedAddition[itemId].atime = time;
								updatedAddition[itemId].isSelf = self;
								updatedAddition[itemId].faster = faster;
							} else {
								updatedAddition[itemId] = {
									atime: time,
									isSelf: self,
									faster: faster,
								};
							...
							setAddition(updatedAddition);
							localStorage.setItem('additionD', JSON.stringify(updatedAddition));
						}}
						addition={addition}
					/>
				</Route>
